...
class Args:
    ...
    # WarehouseSort-specific: demo-seeded replay buffer
    demo_path: Optional[str] = None
    """path to a WarehouseSort rgb demo .h5 (e.g. il/demos/easy/trajectory.rgb...h5). When set,
    the demo's recorded env_kwargs (num_parcels/fixed_poses/randomization/obs_camera) are used to
    construct the training and eval envs, AND the replay buffer is pre-seeded by replaying the
    demo actions through a real env instance to get ground-truth (obs, reward, done) transitions
    before online SAC training starts."""
...
def seed_replay_buffer_from_demos(rb: ReplayBuffer, demo_path: str, env_id: str, env_kwargs: dict,
    ...
    seed_env = gym.make(env_id, num_envs=1, reward_mode="sparse", **env_kwargs)
    seed_env = FlattenRGBDObservationWrapper(seed_env, rgb=True, depth=False, state=True)
...
if __name__ == "__main__":
    ...
    # obs_camera) so training and eval render exactly the distribution the demos came from.
    demo_info = None
    ...
    if not args.evaluate:
        ...
        if args.track:
            import wandb
            config = vars(args)
            config["env_cfg"] = dict(**env_kwargs, num_envs=args.num_envs, env_id=args.env_id, reward_mode="sparse", env_horizon=max_episode_steps, partial_reset=args.partial_reset)
            config["eval_env_cfg"] = dict(**env_kwargs, num_envs=args.num_eval_envs, env_id=args.env_id, reward_mode="sparse", env_horizon=max_episode_steps, partial_reset=False)
            wandb.init(
